...
继承  下级别成员除了拥有上一级的共性，还有自己的特性，可以使用继承技术，减少重复代码   是面向对象编程（OOP）的一个基本概念，允许一个类（子类）继承另一个类（父类）的属性和方法。
基本语法
class Java :public Base{}; 
class 子类：继承方式 父类
子类也称派生类 1.继承于基类，表现为共性2.自己增加的成员，表现为个性  子类可以任意使用父类的公共成员函数
父类也称基类
...
继承中的对象模型
除了静态成员变量/函数、构造函数、析构函数，其他的都会继承到子类（包括私有成员） 利用工具查看类结构：1.F: ;2.cd 具体路径 ;3.cl / dl reportSingleClassLayout类名 源文件
继承中构造和析构的调用顺序：
先构造父类，再构造对象，再构造子类
先析构子类，再析构对象，再析构父类
当派生类的构造函数被调用时，必须首先调用基类的构造函数来初始化基类部分。
如果基类没有默认构造函数（即没有无参数的构造函数），那么派生类的构造函数必须显式地调用基类的构造函数
同名成员处理：
...
友元函数不能被继承，会破坏类的隐藏性和封装性,但是友元函数可以通过派生类对象访问这些继承的私有成员



多态:是面向对象编程中另一个基本概念，它允许你通过父类的引用或指针来调用子类的方法。多态通常通过虚函数实现。
1.静态多态：函数重载、运算符重载属于静态多态
2.动态多态：派生类和虚函数实现运行时多态 满足条件1.有继承关系 2.子类重写父类的虚函数（重写：父类和子类的函数完全一致）   使用条件：父类的指针或者引用指向子类对象 Base*x=new Son();通过基类指针或引用调用虚函数
两者区别：静态多态的函数地址早绑定-编译阶段确认函数地址；动态多态的函数地址晚绑定-运行阶段确认函数地址
虚函数：vfptr虚函数表指针 vftable
总结：当子类重写父类的虚函数时，会覆盖父类中的虚函数
构造函数不能是虚函数，析构函数可以是虚函数
虚函数的机制依赖于虚函数表（vtable），而虚函数表是在对象构造过程中建立的。如果构造函数是虚函数，在对象尚未完全构造之前，虚函数表还不存在，这会导致逻辑上的矛盾
...
一个使用多态行为的类型若有子类，必须要有虚函数（特别是析构），为了避免通过基类指针删除子类对象可能只调用基类的析构函数，导致资源泄露或未完全销毁对象
3.基类指针或引用：通过基类的指针或引用调用派生类的方法。下述为举例
class Base {
public:
    virtual void show() {
...
类模板与友元：
类内实现 friend void show(ppp<nametype,agetype> x){}
类外实现  太复杂了，不要用

STL
六大组件：
容器：数据结构 vector等 顺序容器、关联容器
算法：常用算法 sort等 质变算法、非质变算法
迭代器：类似指针
仿函数：行为类似函数，作为算法的判断条件
适配器：修饰容器或仿函数或迭代器接口的东西
空间配置器：负责空间的配置和管理
string本质是一个类，类内封装了char*，管理这个字符串，是一个char*型的容器 （字符数组就是一个字符串）
...
智能指针：
①独占指针：unique_ptr
* 在任何给定的时刻，智能有一个指针管理内存
* 当指针超出作用域时，内存将自动释放
* 该类型指针不可copy，只能Move
创建方式：
通过已有裸指针创建
通过new创建
通过make_unique创建（推荐）std::unique_ptr<int> ptr = std::make_unique<int>(42);
unique_ptr可以通过get()获得地址
unique_ptr实现了->与*：可以通过->调用成员函数；可以通过*调用dereferencing（解引用）

②std::shared_ptr：std::shared_ptr<int> ptr1 = std::make_shared<int>(20); // 创建一个 shared_ptr
共享所有权，可以在多个 std::shared_ptr 之间共享。
引用计数机制，当最后一个 std::shared_ptr 被销毁时，所管理的对象才会被销毁。
使用场景：需要共享资源的多个对象之间，如图形节点树等。

③std::weak_ptr：std::shared_ptr<int> sharedPtr = std::make_shared<int>(30); // 创建一个 shared_ptr
辅助 std::shared_ptr，不会增加引用计数。
用于解决循环引用问题。
不能直接访问对象，需要通过 std::weak_ptr.lock() 获取一个 std::shared_ptr。
*/
