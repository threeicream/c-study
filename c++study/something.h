...
4. forward_list
定义：forward_list 是单向链表，只支持单向遍历和在头部插入删除元素。 常用成员函数：
...
删除元素：指向被删除元素的迭代器、引用和指针会失效，但其他的不会失效。
 ...
 拷贝算法：
 copy(a.begin(),a.end(),b);//把a的内容拷贝到b中 必须保证a、b两者有一样多的元素（copy无法从容器进行添加/删除操作）
 ...
 unique(e,q);//消除相邻的重复项，返回指向不重复区域之后一个位置的迭代器（说是消除，其实是将其移动到末尾，并将元素等于原来的end）
...
function body：函数体，包含具体的操作。
 ...
 find_each(e,q,Lambda);//对e-q范围内的元素进行操作，看Lambda表达式返回的值
