 ...
 *(v.begin())就是v第一个元素的值
 ...
 显示转换：（一般不推荐）
 1.static_cast:（编译时类型检查）强制转换除了底层const以外的类型(有点鸡肋，指针方面只能针对void)用于相关类型之间的转换，如基本类型之间（如 int 到 float）、类层次结构中基类和派生类之间的转换、以及 void 指针与具体类型指针之间的转换。
 int a = 1;
 void* c = &a;
 ...
 4.dynamic_cast：（运行时类型检查）检查表达式指针指向的对象类型 将基类指针或引用转换为派生类指针或引用
 ...
 形参数量或者形参类型不同才能区分出来
 运算符重载：
 在 C++ 中，运算符重载（operator overloading）允许你为自定义类型（如类）定义新的运算符行为。operator<< 重载是一个常见的例子，它允许你使用 << 运算符来输出自定义类型的对象，就像输出基本数据类型一样。
为什么需要重载 operator<<
默认情况下，C++ 的 << 运算符只能用于输出基本数据类型（如 int、double 等）。通过重载 operator<<，你可以定义如何输出自定义类型的对象，使代码更简洁和易读。
如何重载 operator<<
通常，operator<< 被定义为一个友元函数，因为它需要访问类的私有成员。
...
int main() {
 ...
 这个 const 修饰整个成员函数。它表示这是一个常量const成员函数。这意味着：
...
删除元素：指向被删除元素的迭代器、引用和指针会失效，但其他的不会失效。
 ...
 capture：捕获外部变量的方式，可以是值捕获（=）或引用捕获（&）。//[=,&a][&,a]隐式捕获 值捕获不能修改值，引用捕获则可以
 parameters：函数参数列表。
 ...
 size_t v1=42
 auto f=[v1]()mutable(return ++v1;};
 //auto f=[v1]()mutable(return ++v1;}();这个代表直接调用，就不需要 auto j=f();了
 v1=0
 auto j=f();//j=1
 为lambda定义返回类型：
 ...
 *out,++out,out++				   都只会返回out
 ...
 指向函数的指针定义：
 返回类型(*指针名)(函数参数列表)，例如void(*q)(int)
 指向成员函数的指针则略有不同：
 1.指向非静态成员函数：返回类型(类名::*指针名)(函数参数列表)
 2.指向静态成员函数：返回类型(*指针名)(函数参数列表)

 哈希表：
 unordered_map是一个键值对容器，允许快速查找、插入和删除操作
 声明和初始化
...
删除元素
map.erase(3); // 删除键为3的元素

unordered_set的使用
unordered_set是一个只包含键的容器，适用于需要快速查找唯一元素的场景。
声明和初始化
...
易错点：
 1数组名是指针常量，其地址不可改变
 2数组作为参数传递给函数时，它会退化为指向数组第一个元素的指针
 3宏定义只是简单的文本替换
 4实现动态联编用父类指针或引用指向子类对象
 5  %-6.2e 2.19e+02
 格式说明符解释：
 %e：表示以科学计数法（指数表示法）输出浮点数。
 -：左对齐（表示输出的结果将会靠左对齐，右侧填充空格）。
 6：表示字段宽度为 6 个字符（包括数字、符号、小数点、指数符号等）。
 .2：表示保留两位小数。
 6算术运算符的优先级高于关系运算符，关系运算符的优先级高于逻辑运算符，逻辑运算符的优先级则高于赋值运算符。例如，a=x+y>2&&x+y<8；等价于a=(((x+y)>2)&&((x十y)<8))；
 7二叉搜索树通过中序遍历进行确认（左最小，右最大）
 8修改指针类型参数时，记得加一个& （ListNode*& pre），这样才能修改指针的地址
 */

//C++新特性or函数
//popcount 函数用于计算一个整数的二进制表示中有多少个1
//int qwef = std::popcount(1u * 5 ^ 3);//1u 代表无符号整数 1。
//ranges::max(nums);//C++20 的语法，用于查找一个范围（例如 std::vector 或 std::array）中的最大元素
//std::fill(begin_iterator, end_iterator, value);//将一个指定值填充到某个范围内
//std::partial_sum(first, last, result);//用于计算部分和，即对于一个输入范围，计算其累积和，并将结果存储到另一个范围中
